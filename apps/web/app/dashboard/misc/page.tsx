...
import type { Call, Paginated, MiscCountStats } from "@callflow/shared-types";
...
export default function MiscPage() {
  ...
  const { data: stats } = useSWR<MiscCountStats>("/analytics/misc-count", fetcher);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {/* Page header */}
      <div>
        <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700, color: C.text, letterSpacing: -0.5 }}>Misc Calls</h1>
        <p style={{ margin: "5px 0 0", fontSize: 15, color: C.muted }}>
          Disconnected, no-response, and short calls under 10 secs — excluded from all analytics
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 16 }}>
        ...
        <StatCard
          label="Avg Duration"
          value={stats ? fmtS(stats.avg_duration_secs) : "—"}
          icon="⏱"
          accent={C.blue}
        />
        <StatCard
          label="Disconnected"
          value={stats?.disconnected_count ?? "—"}
          icon="📵"
          accent={C.red}
        />
        <StatCard
          label="No Response"
          value={stats?.no_response_count ?? "—"}
          icon="🔇"
          accent={C.muted}
        />
      </div>
